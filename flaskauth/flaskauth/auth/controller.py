...
def register():
    ...
    # return make_response(jsonify({'error': error}), 400)
        ...
        # Lidar com exceções do Celery durante o envio do e-mail
        error_message = str(e)
        logger.error(f"Celery error during email sending: {error_message}")
        return error({}, error_message, 500)

    except Exception as e:
        # Lidar com outras exceções não previstas
        error_message = str(e)
        logger.error(f"Unexpected error during registration: {error_message}")
        return error({}, error_message, 500)
...
def refreshToken():
    ...
    refresh_token = RefreshToken.query.filter(RefreshToken.token == form_data['refresh_token'],
                                              RefreshToken.expired_at >= now).first()
